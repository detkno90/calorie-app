...
export default function FoodSearch() {
  ...
  async function searchFood(e, loadMore = false) {
    ...
    setSearching(true);
    try {
      const res = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&search_simple=1&action=process&json=1&page_size=20&page=${currentPage}&fields=id,product_name,brands,nutriments,serving_size`);
      const data = await res.json();
      const filtered = (data.products || []).filter(p => p.product_name?.trim() && (p.nutriments?.['energy-kcal_100g'] || p.nutriments?.['energy-kcal_serving']));
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <button
          ...
          onClick={() => { setManualMode(!manualMode); setResults([]); }}
        >
        ...
        <>
          ...
          {searching && results.length === 0 && (
            <div className="empty-state">
              ...
              <div className="empty-state-sub">This may take a few seconds</div>
            </div>
          )}
          ...
          {results.length > 0 && (
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '12px 18px 8px', fontSize: 12, color: 'var(--text2)', fontWeight: 500 }}>
          ...
          {!searching && results.length === 0 && query && (
            <div className="empty-state">
              ...
              <div className="empty-state-sub">Try simpler words or use Manual entry</div>
            </div>
          )}
          ...
          {!query && (
            <div className="empty-state">
              <div className="empty-state-icon">🔍</div>
              <div className="empty-state-title">Search for a food</div>
              <div className="empty-state-sub">Try "banana", "rice", "chicken"</div>
            </div>
          )}
        </>
      )}
        ...
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              {[
                { label: 'Calories', value: Math.round(getCals(selected) * servings), unit: 'kcal', color: '#4CAF82' },
                { label: 'Protein',  value: Math.round(getProtein(selected) * servings), unit: 'g', color: '#EF5B5B' },
                { label: 'Carbs',    value: Math.round(getCarbs(selected) * servings), unit: 'g', color: '#4CAF82' },
                { label: 'Fat',      value: Math.round(getFat(selected) * servings), unit: 'g', color: '#F5A623' },
              ].map(n => (
                <div key={n.label} style={{ flex: 1, background: 'var(--bg3)', borderRadius: 10, padding: '12px 8px', textAlign: 'center' }}>
                  <div style={{ fontSize: n.label === 'Calories' ? 18 : 16, fontWeight: 700, color: n.color }}>{n.value}</div>
                  <div style={{ fontSize: 10, color: 'var(--text2)', marginTop: 2 }}>{n.unit}</div>
                  <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 1 }}>{n.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="input-group">
              ...
              <input
                className="input" type="number" min="0.1" step="0.1"
                value={servings}
